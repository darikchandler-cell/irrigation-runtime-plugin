...
class Irrigation_Calculator {
    ...
    public function render_calculator($atts) {
        ...
        // Use output buffering to ensure scripts are enqueued even if wp_enqueue_scripts already ran
        if (!did_action('wp_enqueue_scripts')) {
            // Scripts haven't been enqueued yet, so this will work normally
            $this->enqueue_assets();
        } else {
            // Scripts already enqueued, but we need to force load our assets
            // This handles cases where shortcode is rendered after wp_enqueue_scripts
            $js_file = IRRIGATION_CALC_PLUGIN_DIR . 'build/app.js';
            $css_file = IRRIGATION_CALC_PLUGIN_DIR . 'build/app.css';
            
            if (file_exists($js_file) && file_exists($css_file)) {
                // Enqueue if not already enqueued (WordPress will handle duplicates)
                wp_enqueue_script(
                    'irrigation-calculator-app',
                    IRRIGATION_CALC_PLUGIN_URL . 'build/app.js',
                    array(),
